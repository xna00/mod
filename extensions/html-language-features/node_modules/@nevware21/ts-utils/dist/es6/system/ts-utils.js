...
System.register('nevware21.ts-utils', [], (function (exports) {
    ...
    return {
        execute: (function () {
            ...
            const isPrimitiveType = exports('isPrimitiveType', (theType) => {
                return theType !== OBJECT && PRIMITIVE_TYPES.indexOf(theType) !== -1;
            });
            const isString = exports('isString', _createIs(STRING));
            const isFunction = exports('isFunction', _createIs(FUNCTION));
            ...
            const isDate = exports('isDate', _createObjIs("Date"));
            const isNumber = exports('isNumber', _createIs(NUMBER));
            const isBoolean = exports('isBoolean', _createIs(BOOLEAN));
            const isRegExp = exports('isRegExp', _createObjIs("RegExp"));
            const isFile = exports('isFile', _createObjIs("File"));
            const isFormData = exports('isFormData', _createObjIs("FormData"));
            const isBlob = exports('isBlob', _createObjIs("Blob"));
            const isArrayBuffer = exports('isArrayBuffer', _createObjIs("ArrayBuffer"));
            const isError = exports('isError', _createObjIs("Error"));
            ...
            const strSlice = exports('strSlice', _unwrapFunction(SLICE, StrProto));

            const strSubstring = exports('strSubstring', _unwrapFunction("substring", StrProto));
            const strSubstr = exports('strSubstr', _unwrapFunctionWithPoly("substr", StrProto, polyStrSubstr));
            ...
            const strStartsWith = exports('strStartsWith', _unwrapFunctionWithPoly("startsWith", StrProto, polyStrStartsWith));
            ...
            let _isWebWorker;
            let _isNode;
            const lazySafeGetInst = exports('lazySafeGetInst', (name) => safeGetLazy(() => getInst(name) || UNDEF_VALUE, UNDEF_VALUE));
            const getGlobal = exports('getGlobal', (useCached) => {
                (!_cachedGlobal || useCached === false || (_globalLazyTestHooks && _globalLazyTestHooks.lzy && !_cachedGlobal.b)) && (_cachedGlobal = safeGetLazy(_getGlobalValue, NULL_VALUE));
                return _cachedGlobal.v;
            });
            const getInst = exports('getInst', (name, useCached) => {
            ...
            const hasDocument = exports('hasDocument', () => !!getDocument());
            const getDocument = exports('getDocument', () => {
                (!_cachedDocument || (_globalLazyTestHooks && _globalLazyTestHooks.lzy && !_cachedDocument.b)) && (_cachedDocument = lazySafeGetInst("document"));
                return _cachedDocument.v;
            });
            const hasWindow = exports('hasWindow', () => !!getWindow());
            const getWindow = exports('getWindow', () => {
                (!_cachedWindow || (_globalLazyTestHooks && _globalLazyTestHooks.lzy && !_cachedWindow.b)) && (_cachedWindow = lazySafeGetInst(WINDOW));
                return _cachedWindow.v;
            });
            const hasNavigator = exports('hasNavigator', () => !!getNavigator());
            const getNavigator = exports('getNavigator', () => {
                (!_cachedNavigator || (_globalLazyTestHooks && _globalLazyTestHooks.lzy && !_cachedNavigator.b)) && (_cachedNavigator = lazySafeGetInst("navigator"));
                return _cachedNavigator.v;
            });
            const hasHistory = exports('hasHistory', () => !!getHistory());
            const getHistory = exports('getHistory', () => {
            ...
            const isWebWorker = exports('isWebWorker', () => {
            ...
            const isSymbol = exports('isSymbol', _createIs("symbol"));
            function hasSymbol() {
                return !!getSymbol();
            }
            function getSymbol() {
            ...
            const fnApply = exports('fnApply', _unwrapInstFunction("apply"));
            ...
            const arrEvery = exports('arrEvery', _unwrapFunction("every", ArrProto));
            const arrFilter = exports('arrFilter', _unwrapFunction("filter", ArrProto));
            ...
            const arrIndexOf = exports('arrIndexOf', _unwrapFunction(INDEX_OF, ArrProto));
            const arrLastIndexOf = exports('arrLastIndexOf', _unwrapFunction(LAST_INDEX_OF, ArrProto));

            const arrMap = exports('arrMap', _unwrapFunction("map", ArrProto));

            const arrSlice = exports('arrSlice', _unwrapFunction(SLICE, ArrProto));

            const fnCall = exports('fnCall', _unwrapInstFunction("call"));
            ...
            const arrFind = exports('arrFind', _unwrapFunctionWithPoly("find", ArrProto, polyArrFind));
            const arrFindIndex = exports('arrFindIndex', _unwrapFunctionWithPoly("findIndex", ArrProto, polyArrFindIndex));
            const arrFindLast = exports('arrFindLast', _unwrapFunctionWithPoly("findLast", ArrProto, polyArrFindLast));
            const arrFindLastIndex = exports('arrFindLastIndex', _unwrapFunctionWithPoly("findLastIndex", ArrProto, polyArrFindLastIndex));

            const arrFrom = exports('arrFrom', ArrCls.from || polyArrFrom);

            const arrIncludes = exports('arrIncludes', _unwrapFunctionWithPoly("includes", ArrProto, polyArrIncludes));
            const arrContains = exports('arrContains', arrIncludes);

            const arrReduce = exports('arrReduce', _unwrapFunction("reduce", ArrProto));

            const arrSome = exports('arrSome', _unwrapFunction("some", ArrProto));

            const fnBind = exports('fnBind', _unwrapInstFunction("bind"));

            const createFnDeferredProxy = exports('createFnDeferredProxy', (hostFn, funcName) => {
                return function () {
            ...
            const createProxyFuncs = exports('createProxyFuncs', (target, host, funcDefs) => {
            ...
            let _iterSymbol;
            const readArgs = exports('readArgs', (theArgs, start, end) => {
                if (!_iterSymbol) {
            ...
            const objCreate = exports('objCreate', _objCreate || polyObjCreate);
            ...
            const polyStrTrim = exports('polyStrTrim', _createTrimFn(/^\s+|(?=\s)\s+$/g));
            const polyStrTrimStart = exports('polyStrTrimStart', _createTrimFn(/^\s+/g));
            const polyStrTrimEnd = exports('polyStrTrimEnd', _createTrimFn(/(?=\s)\s+$/g));

            const strTrim = exports('strTrim', _unwrapFunctionWithPoly("trim", StrProto, polyStrTrim));
            const strTrimStart = exports('strTrimStart', _unwrapFunctionWithPoly("trimStart", StrProto, polyStrTrimStart));
            const strTrimLeft = exports('strTrimLeft', strTrimStart);
            const strTrimEnd = exports('strTrimEnd', _unwrapFunctionWithPoly("trimEnd", StrProto, polyStrTrimEnd));
            const strTrimRight = exports('strTrimRight', strTrimEnd);

            const strUpper = exports('strUpper', _unwrapFunction("toUpperCase", StrProto));
            const strLower = exports('strLower', _unwrapFunction("toLowerCase", StrProto));
            ...
            const mathTrunc = exports('mathTrunc', _mathTrunc ? _mathTrunc : polyMathTrunc);
            ...
            const strRepeat = exports('strRepeat', _unwrapFunctionWithPoly("repeat", StrProto, polyStrRepeat));
            ...
            const strPadStart = exports('strPadStart', _unwrapFunctionWithPoly("padStart", StrProto, polyStrPadStart));
            const strPadEnd = exports('strPadEnd', _unwrapFunctionWithPoly("padEnd", StrProto, polyStrPadEnd));
            ...
            const getLength = exports('getLength', _unwrapProp(LENGTH));
            ...
            const strSplit = exports('strSplit', _unwrapFunction("split", StrProto));
            const strSymSplit = exports('strSymSplit', hasSymbol() ? _unwrapFunction("split", StrProto) : polyStrSymSplit);
            ...
            const strEndsWith = exports('strEndsWith', _unwrapFunctionWithPoly("endsWith", StrProto, polyStrEndsWith));
            ...
            const strIndexOf = exports('strIndexOf', _unwrapFunction(INDEX_OF, StrProto));
            const strLastIndexOf = exports('strLastIndexOf', _unwrapFunction(LAST_INDEX_OF, StrProto));

            const strIncludes = exports('strIncludes', _unwrapFunctionWithPoly("includes", StrProto, polyStrIncludes));
