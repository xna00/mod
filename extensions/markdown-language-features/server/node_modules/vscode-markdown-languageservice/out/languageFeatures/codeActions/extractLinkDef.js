...
class MdExtractLinkDefinitionCodeActionProvider {
    ...
    #getExtractLinkAction(doc, linkInfo, targetLink) {
        ...
        for (const link of linkInfo.links) {
            if (link.kind === documentLinks_1.MdLinkKind.Link || link.kind === documentLinks_1.MdLinkKind.AutoLink) {
                if (this.#matchesHref(targetLink.href, link)) {
                    const targetRange = link.kind === documentLinks_1.MdLinkKind.AutoLink ? link.source.range : link.source.targetRange;
    ...
    #getLinkTargetText(doc, link) {
        const afterHrefRange = link.kind === documentLinks_1.MdLinkKind.AutoLink
            ? link.source.targetRange
            : (0, range_1.makeRange)((0, position_1.translatePosition)(link.source.targetRange.start, { characterDelta: 1 }), (0, position_1.translatePosition)(link.source.targetRange.end, { characterDelta: -1 }));
