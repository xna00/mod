...
type typed_tm = { tm : tm_desc; ty : ty; env : env }
[@@deriving show { with_path = false }]
...
let rec gen_constr env f t : typed_tm * uvargenerator * constr =
  match t with
  | TmBool -> ({ env; tm = Tbool; ty = TyBool }, f, [])
  | TmInt -> ({ env; tm = Tint; ty = TyInt }, f, [])
  | TmVar x ->
      let ty =
  ...
  | TmAbs (x, Some ty1, t2) ->
      ...
      ({ env; tm = Tabs (x, Some ty1, ty2); ty = TyArr (ty1, ty2.ty) }, f, c)
  | TmAbs (x, None, t2) ->
      let (NextUVar (name, f)) = f () in
      let ty_x = TyId name in
      let ty, f, c = gen_constr ((x, ty_x) :: env) f t2 in
      ({ env; tm = Tabs (x, None, ty); ty = TyArr (ty_x, ty.ty) }, f, c)
  | TmApp (t1, t2) ->
      ...
      ({ env; tm = Tapp (ty1, ty2); ty = ty_r }, f, c)
  | TmLet (r, x, t1, t2) ->
      let typed_tm, f, c1 =
        if r then
      ...
      let ty2, f, c2 =
        gen_constr ((x, if is_value t1 then gen env ty1 else ty1) :: env) f t2
      in
      ({ env; tm = Tlet (r, x, typed_tm, ty2); ty = ty2.ty }, f, c1 @ c2)

let rec subst_typed s typed =
  let { env; tm; ty } = typed in
  {
...
let tif =
  TmAbs
    ( "c",
      Some TyBool,
      TmAbs ("x", Some (TyId "A"), TmAbs ("y", Some (TyId "A"), TmVar "y")) )

let tid = TmAbs ("x", None, TmVar "x")
    ...
    ( true,
      ...
      TmAbs
        ( "y",
          None,
          TmApp
            (TmApp (TmApp (tif, TmBool), TmInt), TmApp (TmVar "x", TmVar "y"))
        ),
      TmVar "x" )

let _ =
  let typed, _, c = gen_constr [] uvargen tid in
  let s = unify c in
  print_endline (print_typed (subst_typed s typed));
  print_endline (show_constr c)
